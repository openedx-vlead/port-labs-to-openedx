...
* Introduction
  ...
  Currently there are around 150 labs which have been developed by
  various institutes.The authoring of Virtual Labs in Open edX
  platform described here defines the creation and hosting of a new
  lab in the Open edX platform. It aims at delivering responsive,
  open-source and device-independent labs, helping us in our strive
  for excellence.
* Motivation
...
** Request of a new lab
   + A new lab request should be raised as an issue in Virtual-Labs
     engineers-forum in GitHub.
** Creation of lab specifications
   Central Team would create the lab specifications in the json format
   and push in the [[https://github.com/openedx-vlead/lab-specifications][GitHub repo]]. The below steps would be followed by
   them to create the required structure of the lab for Open edX :
...
** Creation of lab by the author
   + [[Lab contents ?? A virtual Lab has generally following sections,
     links]] ...
   + The lab specification file link would be provided to the lab
     author and he/she would create the course referring to the given
     values :
     + Course Name
     + Organisation Name
     + Course ID
     + Course Start Date
   + The course creator would be given the staging instance of Open
     edX platform to create the lab.
   + [[http://open-edx.vlabs.ac.in:18010/][Open edX Studio]] have to be signed in to create the lab which is
     ...
     created lab in, [[http://open-edx.vlabs.ac.in][Open edX LMS]] (Learning Management System)
     should be visited.
...
*** Including the images
     + Direct copy pasting the image works sometimes and sometimes
       doesn't. In that case, images can be uploaded from =Content= tab
...
*** Creating the lab with database
