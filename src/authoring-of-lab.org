...
* Disclaimer
  ...
  lab specifications called labspec.json files. They would provide the
  labspecs and the information provided must be adhered by the lab
  author while creating a lab on Open edX. Subsequent to the
  availability of labspecs, each author would be responsible for the
  content of a lab. Also subsequent to the lab creation on the staging
  instance by the author, the hosting request for the lab on the
  production edX environment should be raised in the engineers-forum
  after the initial credentials of a course are as per the lab
  specifications. The protocols for raising requests for hosting must
  be adhered consistenly by all the authors and other modes for the
  communication would not be entertained.
* Summary
