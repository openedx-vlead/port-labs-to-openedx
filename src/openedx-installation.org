...
16 rows in set (0.00 sec)
  ...
  13. Set up pipeline
      ...
      #+END_EXAMPLE
  14. Check the pipeline install by running a simple job to count
      events per day.
      #+BEGIN_EXAMPLE
  ...
  16. Grant =read_only= user privileges to login remotely to LMS
      ...
      sudo /etc/init.d/mysql restart
      #+END_EXAMPLE
  17. Check pipeline install.
      #+BEGIN_EXAMPLE
      # Ensure you're in the pipeline virtualenv 
      remote-task --host localhost --repo https://github.com/edx/edx-analytics-pipeline --user ubuntu --override-config $HOME/edx-analytics-pipeline/config/devstack.cfg --wheel-url http://edx-wheelhouse.s3-website-us-east-1.amazonaws.com/Ubuntu/precise --remote-name analyticstack --wait TotalEventsDailyTask --interval 2015 --output-root hdfs://localhost:9000/output/ --local-scheduler
      #+END_EXAMPLE
  18. Finish rest of the pipeline config
      + Test setup
        #+BEGIN_EXAMPLE
      ...
      + If success
        ...
        - status ::
          ...
          Bye
          #+END_EXAMPLE
  19. Run following on *lms machine*, set =INSIGHTS_BASE_URL= variable
	...
	HINT: Use GenericIPAddressField instead.
{
  ...
  Superuser created successfully.
      #+END_EXAMPLE
  21. Now login with admin credentials to studio from
      http://54.233.130.215:18010
  22. Then go to http://54.233.130.215/admin/ to view admin console.
  23. Check it
      #+BEGIN_EXAMPLE
