...
** How to delete Open edX Studio user from database
   ...
   #+BEGIN_EXAMPLE
   delete from auth_userprofile where user_id=
   #+END_EXAMPLE
...
** TODO Configure edX platform so that it gives us proper verification link
   - Edit the file
     =/edx/app/edx_ansible/edx_ansible/playbooks/roles/edxapp/defaults/main.yml=
     and Modify the =EDXAPP_SITE_NAME=
     #+BEGIN_EXAMPLE
     EDXAPP_SITE_NAME = 'courses.vlabs.ac.in or <public IP>'
     #+END_EXAMPLE
   - Restart ngnix
     #+BEGIN_EXAMPLE
     /etc/init.d/ngnix restart
     #+END_EXAMPLE
   - Run the following commands to restart the edx services not just
     ngnix
     #+BEGIN_EXAMPLE
     ...
     /edx/app/edxapp/edx-platform# /edx/bin/supervisorctl -c /edx/etc/supervisord.conf restart edxapp_worker:
