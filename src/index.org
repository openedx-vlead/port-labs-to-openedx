1;2501;0c#+TITLE: Porting Virtual Labs to Open edX Platform
#+Author: VLEAD
#+Date: [2016-01-27 Wed]
...
* Taking backup and restore from this backup
** Take backup of existing database from Kifli release and restore it into cypress release
   - Took backup of existing databases which is running on klfli
     release.  For this I ran following commands
     #+BEGIN_SRC 
     mongodump -o ./mongo-backup
     mysqldump -u root -p --all-databases > ./backup.sql
     #+END_SRC
   - Copied backed up databases to newly created Open edX platform
     which is running on cypress.
   - Tried to restore the mogodatabase into cypress
     #+BEGIN_SRC 
     mongorestore -drop ./mongo-backup     
     #+END_SRC
     + restored without any issue
   - Tried to restore mysql database into cypress 
     #+BEGIN_SRC 
     mysql -u root -p < ./backup.sql
     #+END_SRC
     + It thrown error with the bellow lines
       #+BEGIN_EXAMPLE
       sudo mysql -uedxapp001 -p < ./backup.sql
       Enter password: 
       ERROR 1215 (HY000) at line 2465: Cannot add foreign key constraint
       #+END_EXAMPLE
   - This could not migrate database into new release of Open edX
     platform. This may be due to updating database from older schema
     to new schema.
** TODO Take backup of cypress and restore it into newly created edX platform which has cypress release
   - Take backup 
   - Save 
   - Launch new VM with required configuration (Ubuntu 12.04)
   - In Progress - Setup Open edX platform with cypress on that VM 
...
* Open Questions
  1. How does a database used by a lab integrated into Open edX platform?
